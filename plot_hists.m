function [h1, h2, h3, h4, h5, h6, h7, h8, fig1, fig2] = plot_hists(models_pbi, poisson_saved, Z_saved)
    ...
    fig1 = figure;
    subplot(2,3,1);
    h1=histogram(models_pbi(:,1), 'BinWidth',10, 'Normalization','pdf');
    xlabel('{rho} 1 (kg/m3)');
    ylabel('pdf')
    xlim([0 4000]);
    inv_figure
    ...
    subplot(2,3,2);
    h2 = histogram(models_pbi(:,3), 'BinWidth',10, 'Normalization','pdf');
    xlabel('Vp 1 (m/s)');
    ylabel('pdf')
    xlim([0 8000]);
    inv_figure
    ...
    subplot(2,3,3);
    h3=histogram(models_pbi(:,5), 'BinWidth',10, 'Normalization','pdf');
    xlabel('Vs 1 (m/s)');
    ylabel('pdf')
    xlim([0 3000]);
    inv_figure
    ...
    subplot(2,3,4)
    h4=histogram(models_pbi(:,2), 'BinWidth',25, 'Normalization','pdf');
    xlabel('{rho} 2 (kg/m3)');
    ylabel('pdf')
    xlim([0 4000]);
    inv_figure
    ...
    subplot(2,3,5)
    h5=histogram(models_pbi(:,4), 'BinWidth',25, 'Normalization','pdf');
    xlabel('Vp 2 (m/s)');
    ylabel('pdf')
    xlim([0 8000]);
    inv_figure
    ...
    subplot(2,3,6)
    h6=histogram(models_pbi(:,6), 'BinWidth',25, 'Normalization','pdf');
    xlabel('Vs 2 (m/s)');
    ylabel('pdf')
    xlim([0 3000]);
    inv_figure
    ...
    h7=histogram(poisson_saved,edges, 'Normalization','pdf');
    xlim([0,0.5]);
    ...
    subplot(1,2,2)
    h8=histogram(Z_saved, 'BinWidth',100000, 'Normalization','pdf');
